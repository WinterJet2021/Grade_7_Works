...
char detectLines()
{
	...
	barcodeNumbers = letters[barcdeNumbers];
	return barcodeLetters;

}

void goForward()
{
	if (barcodeNumbers > 13)
	{
		forward(3, rotations, 30);
		turnRight(180, degrees, 30);
		forward(4, rotations, 30);
	}
	else
	{
		forward(3, rotations, 30);
		turnLeft(180, degrees, 30);
		forward(4, rotations, 30);
	}
}
...
char scannedLetter;
scannedLetter = detectLines();
displayTextLine(1, " %c ", scannedLetter);
sleep(1000);
backward(0.1, rotations, 50);
moveMotor(motorA, 100, degrees, 20);
detectLines();
goForward();
